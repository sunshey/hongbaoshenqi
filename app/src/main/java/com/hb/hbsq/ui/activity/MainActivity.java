...
import com.hb.hbsq.adapter.MainFunctionAdapter;
...
import com.hb.hbsq.listener.OnUpdateListener;
import com.hb.hbsq.service.HbService;
import com.hb.hbsq.ui.fragment.dialog.ExceptionalFragment;
import com.hb.hbsq.ui.fragment.dialog.OpAsFragment;
import com.hb.hbsq.ui.fragment.dialog.TwoButtonDialog;
import com.hb.hbsq.util.AppUpdateManager;
import com.hb.hbsq.util.Encrypt;
import com.hb.hbsq.util.LogUtil;
import com.hb.hbsq.util.NetUtils;
import com.hb.hbsq.util.SMSUtil;
import com.hb.hbsq.util.SPUtils;
import com.hb.hbsq.util.ToastUtil;
import com.hb.hbsq.util.UIUtils;
import com.hb.hbsq.widget.AutoRollTextView;
import com.hb.hbsq.widget.CircleScanView;
import com.hb.hbsq.widget.FllowerAnimation;
...
public class MainActivity extends AppCompatActivity {
    ...
    TextView tvState;
    @BindView(R.id.btn_vip_speed)
    Button btnVipSpeed;
    @BindView(R.id.btn_svip)
    Button btnSvip;
...
//        UmengTool.checkWx(this);
    ...
    private void init() {
        rcvMainFunction.setLayoutManager(new LinearLayoutManager(this));
        String[] items = getResources().getStringArray(R.array.main_function);
        List<String> mList = new ArrayList<>(Arrays.asList(items));
        MainFunctionAdapter adapter = new MainFunctionAdapter(this, mList);
        rcvMainFunction.setAdapter(adapter);
    }

    @OnClick({R.id.iv_vip, R.id.iv_caishen})
        ...
        switch (view.getId()) {
            ...
            case R.id.iv_caishen://点击财神，扫描扇区出现
                if (HbService.isRunning()) {
                    ...
                    ToastUtil.showToast(this, "点击[乐乐红包神器 ❤❤开启有效❤❤]开启服务", Toast.LENGTH_SHORT);
                }
                break;
        }
    }
    ...
    private void getData() {
        ...
        loginEngin.rxGetInfo().observeOn(AndroidSchedulers.mainThread()).subscribe(new Action1<ResultInfo<LoginDataInfo>>() {
            ...
            public void call(final Throwable throwable) {
                ...
                ToastUtil.showToast(MainActivity.this, throwable.getMessage(), Toast.LENGTH_SHORT);
    ...
    private void update(File dir) {
        AppUpdateManager.downloadFile(this, GoagalInfo.loginDataInfo.getUpdateInfo().getUrl(), dir, new OnUpdateListener() {
            ...
            public void onSuccess(final File file) {
                if (isNewVersion(file)) {
                    AppUpdateManager.showHintDialog(MainActivity.this, file);
                }
            }
